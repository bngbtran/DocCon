...
pdf_path = sys.argv[1] if len(sys.argv) > 1 else r"C:\Users\bngbtran\Downloads\Eng_Sample.pdf"
...
for bi, block in enumerate(data["blocks"]):
    ...
    print(f"  [B{bi}] y={bbox[1]:.0f}-{bbox[3]:.0f}  x={bbox[0]:.0f}-{bbox[2]:.0f}  cx={cx:.0f}"
...
for di, d in enumerate(drawings[:20]):
    ...
    if r:
        w = r[2]-r[0]; h = r[3]-r[1]
        if h < 3:
            print(f"  [D{di}] HLINE y={r[1]:.1f} x={r[0]:.1f}-{r[2]:.1f} w={w:.1f} h={h:.2f}")
